...
function readQuote(day) {
    db.collection("quotes").doc(day)                                                      //name of the collection and documents should matach excatly with what you have in Firestore
        .onSnapshot(tuesdayDoc => {                                                               //arrow notation
            ...
            document.getElementById("quote-goes-here").innerHTML = tuesdayDoc.data().quote;      //using javascript to display the data on the right place - gives specific key-value pair
...
// readQuote("tuesday");        //calling the function - hard coding it for today
//already called in doAll()
...
// insertNameFromFirestore()
// already called in doAll()
// no need to check because already did in doAll()
...
function displayCardsDynamically(collection) {
    ...
    db.collection(collection).orderBy("length").limit(3).get() //the collection called "hikes"
        .then(allHikes => {
        ...
        if (bookmarks.includes(docID)) {
            currentUser.update({
                ...
                .then(function () {
                    ...
                    //console.log(iconID);
                    //this is to change the icon of the hike that was saved to "not filled"
                    document.getElementById(iconID).innerText = 'bookmark_outline';
                });
        } else {
            currentUser.set({
